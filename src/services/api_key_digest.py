...
def key_id(api_key: str) -> str:
    ...
    return hmac.digest(_pepper(), api_key.encode("utf-8"), "sha256").hex()
...
def migrated_v2_key_id(api_key: str) -> str:
    ...
    old_identifier = hashlib.sha256(api_key.encode("utf-8"), usedforsecurity=False).hexdigest()
    return hmac.digest(_pepper(), old_identifier.encode("ascii"), "sha256").hex()
...
def rate_limit_id(client_id: str) -> str:
    ...
    return hmac.digest(_pepper(), client_id.encode("utf-8"), "sha256").hex()
